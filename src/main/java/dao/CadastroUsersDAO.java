...
public class CadastroUsersDAO {
  ...
  public boolean cadastrar(CadastroUsuarioModel user) {
    ...
    String sql = "INSERT INTO users " +
            "(username, passw, nome, sobrenome, dtaNascimento, matricula, cpf, sexo, email, telefone, funcao, cep, endereco, bairro, cidade, estado, numero, complemento) " +
            "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    try (var con = ConnectionFactory.getConnection()){
        ...
        stmt.setString(2, user.getSenha());       // 2. passw
